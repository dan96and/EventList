...
class EditEventInteractor(val presenter: EditEventPresenter):EditEventInterface.EditEventInteractor {
    ...
    override fun uploadChangesEvent(event: Event) {
        Util.db.collection(Util.userId).document(event.idEvent.toString()).update("name", event.name, "date", event.date,"typeEvent", event.typeEvent, "notification", event.notification ).addOnSuccessListener {
